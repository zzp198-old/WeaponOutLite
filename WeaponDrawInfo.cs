...
namespace WeaponOut
{
    static class WeaponDrawInfo
    {
        ...
        internal static void drawGlowLayer(DrawData data, Player drawPlayer, Item heldItem, PlayerDrawSet drawInfo)
        {
        ...
        internal static DrawData modDraw_HandWeapon(DrawData data, Player p, float length, float width, bool isYoyo = false)
        {
            ...
            if (playerBodyFrameNum < 5) //standing
            {
                ...
                data.position +=
                    new Vector2((4 - length * 0.1f) * p.direction,
                        (width * 0.3f - 4 + 13) * p.gravDir); //back and down;
            ...
            else if (playerBodyFrameNum == 5) //jumping
            {
                ...
                data.position +=
                    new Vector2((width * 0.5f - 8 - length / 2) * p.direction, -14 * p.gravDir); //back and down;
            }
            else //walk cycle base
            {
                data.rotation = (float)(Math.PI * 0.2d * p.direction) * p.gravDir; //rotate 90 clockwise
                data.position += new Vector2((-5 + width * 0.5f) * p.direction,
                    (width * 0.5f - 8 + 14 - length / 2) * p.gravDir);
        ...
        internal static DrawData modDraw_WaistWeapon(DrawData data, Player p, float length)
        {
            ...
            data.rotation =
                (float)(Math.PI * 1 + Math.PI * (0.1f + maxFall * 0.01f) * p.direction) *
                p.gravDir; //rotate just over 180 clockwise
        ...
        {
            data.rotation =
                (float)((Math.PI * 1.1f + Math.PI * (length * -0.001f)) * p.direction) *
                p.gravDir; //rotate just over 180 clockwise
        ...
        internal static DrawData modDraw_AimedWeapon(DrawData data, Player p, float length)
        {
            ...
            else if (playerBodyFrameNum == 5) //jumping
            {
                ...
                data.position +=
                    new Vector2((-10 - length * 0.2f) * p.direction, (10 - length / 2) * p.gravDir); //back and down;
            }
            else //walk cycle base
            {
        ...
        internal static DrawData modDraw_HeavyWeapon(DrawData data, Player p, float width)
        {
            ...
            {
                data.rotation = (float)(Math.PI * 0.008d) * (width * 0.2f + rotationWalkCycle(playerBodyFrameNum) * 6) *
                                p.direction * p.gravDir;
                data.position.X += 8 * p.direction; //forward
